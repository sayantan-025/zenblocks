---
title: Auth Model
description: "A production-ready authentication card with ZenBlocks branding and polished multi-provider support."
icon: LogIn
---
...
The `AuthModel` is a robust, production-level authentication component that retains the iconic ZenBlocks minimalist design. Unlike traditional modals, the `AuthModel` is a standalone card, providing flexibility to be used as a full-page login, a sidebar element, or wrapped inside a `ModalDialog`.

---

## Features

- **Standalone Design**: A clean, card-based layout that works anywhere in your UI.
...
<Preview link="auth-model" useIframe height="600px" />
...
Ensure you have `button` and `input` (shadcn) installed.
...
Copy this into `components/zenblocks/auth-model.tsx`.
...
```tsx
import { AuthModel } from "@/components/zenblocks/auth-model";
import { ModalDialog, ModalContent } from "@/components/zenblocks/modal-dialog";

export default function ModalExample({ open, setOpen }) {
  return (
    <ModalDialog open={open} onOpenChange={setOpen}>
      <ModalContent className="max-w-md p-0 overflow-hidden border-none">
        <AuthModel
          providers={providers}
          className="border-none shadow-none" // Remove card styles when inside modal
        />
      </ModalContent>
    </ModalDialog>
  );
...
| -------------------- | ----------------------- | --------------------- | ------------------------------------------------- |
| title                | ReactNode               | "To use ZenBlocks..." | Component title                                   |
...
- The design is highly flexible: use it as a standalone piece or integrate it into any container.
- Built with standard ZenBlocks aesthetics for a premium, production-ready feel.
- Fully accessible with forwardRef and appropriate ARIA labels.
