---
title: "Logo Loop"
description: "An infinite scrolling marquee with 3D perspective tilt and pause-on-hover effects."
icon: Infinity
---
...
**Logo Loop** creates a smooth, infinite scrolling marquee of logos or icons. It features a unique 3D tilt effect on hover, realistic glint reflections, and automatic pausing for better user interaction. Ideal for "Trusted By" sections or technology stack showcases.

---

## Features

-   **Infinite Scrolling**: Seamless loop using `requestAnimationFrame`.
-   **3D Perspective Tilt**: Interactive mouse-aware tilt on individual items.
-   **Glint Reflections**: Dynamic light sweep effect on hover.
-   **Auto-Pause**: Stops scrolling when user hovers to allow interaction.
-   **Responsive**: Automatically calculates required copies to fill screen width.
...
By default, the component loops through a set of placeholder technology icons.
...
Pass an array of React nodes or image URLs to the `items` prop:
...
export default function Partners() {
  return (
    <LogoLoop 
      items={[
          { node: <Cpu />, name: "Company A" },
          { node: <Globe />, name: "Global Corp" }
      ]} 
    />
  );
}
...
You can also use image assets instead of icons:
...
| :--- | :--- | :--- | :--- |
| `items` | `LogoItem[]` | `DEFAULT_LOGOS` | Array of logos (nodes or images) to display. |
| `speed` | `number` | `100` | Scrolling speed in pixels per second. |
| `direction` | `'left' \| 'right'` | `'left'` | Scroll direction. |
| `gap` | `number` | `24` | Gap between logo items (px). |
| `logoHeight` | `number` | `40` | Height constraint for logo containers (px). |
...
| :--- | :--- | :--- |
| `src` | `string` | Image source URL (if using image). |
| `node` | `ReactNode` | React component/icon (prioritized over src). |
| `alt` | `string` | Alt text for accessibility. |
| `name` | `string` | Optional name shown on hover. |
...
### Animation Config
The smooth velocity transition (for start/stop and hover pause) is controlled by the `ANIMATION_CONFIG` constant at the top of the file.

```tsx
const ANIMATION_CONFIG = {
    SMOOTH_TAU: 0.2, // Time constant for velocity smoothing (Lower = snappier)
    MIN_COPIES: 2,   // Minimum number of loops to render
} as const;
```

### Sensitivity
The `LogoBox` sub-component handles the 3D tilt effect. You can adjust the `useSpring` stiffness and damping in `LogoBox` to make the tilt more or less responsive to mouse movement.
